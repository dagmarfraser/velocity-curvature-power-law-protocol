...
if paramChoice == 1
    ...
    % Font sizes for Maoz (single plot)
    baseFontSize = 14;
    titleFontSize = 18;
    ...
    % Font sizes for Schaal (two plots) - slightly larger
    baseFontSize = 28;
    titleFontSize = 28;
...
if paramChoice == 1
    ...
    for regIdx = 3:5
        ...
        lineHandles(regIdx-2) = plot(noise(1:noiseLength), meanLine, ...
             lineStyles{regIdx-2}, 'Color', colors(regIdx-2,:), 'LineWidth', lineWidth);
    end
    ...
    title({'Butterworth Filter', 'Exhibiting Spurious 1/3 Power Law'}, 'FontSize', titleFontSize)
    ...
    title({'Savitzky-Golay Filter', 'Avoiding Spurious 1/3 Power Law'}, 'FontSize', titleFontSize)
end
...
for i = 1:length(ax)
    ...
    yline(1/3, '-', 'LineWidth', lineWidth, 'Color', [0.2 0.2 0.8]);  % Changed from -1/3
    text(min(noise), 1/3, '  β = 1/3', 'VerticalAlignment', 'bottom', ...  % Changed from -1/3
         'FontWeight', 'bold', 'Color', [0.2 0.2 0.8], 'FontSize', referenceFontSize, 'Interpreter', 'tex');
    ...
    plot([min(noise) max(noise)], [upperBound upperBound], ':', ...
         'Color', [0.2 0.2 0.8], 'LineWidth', lineWidth*0.8);
    plot([min(noise) max(noise)], [lowerBound lowerBound], ':', ...
         'Color', [0.2 0.2 0.8], 'LineWidth', lineWidth*0.8);
    
    text(min(noise), upperBound, '  +10%', 'VerticalAlignment', 'bottom', ...
         'Color', [0.2 0.2 0.8], 'FontSize', referenceFontSize, 'Interpreter', 'tex');
    ...
    grid off
    box on
    xlabel('Noise Magnitude (mm)', 'FontSize', baseFontSize)
    ylabel('Power Law β Value', 'FontSize', baseFontSize, 'Interpreter', 'tex')  % Updated label
...
% Add main title
if paramChoice == 1
    title(plotTitle, 'FontSize', mainTitleFontSize, 'FontWeight', 'bold');
else
    if verLessThan('matlab', '9.11')
        sgtitle(plotTitle, 'FontSize', mainTitleFontSize, 'FontWeight', 'bold');
    else
        title(t, plotTitle, 'FontSize', mainTitleFontSize, 'FontWeight', 'bold');
    end
end
...
annotation('textbox', [0.15 0.85 0.3 0.1], ...
    ...
    'EdgeColor', 'none', 'FitBoxToText', 'on', 'FontSize', annotationFontSize);
else
